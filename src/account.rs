use crate::{
    ...
    libra::Libra,
    options::Options,
    types::{AccountBalanceRequest, AccountBalanceResponse, Amount, BlockIdentifier, Currency}, error::ApiError,
};
use log::debug;
use warp::Filter;
...
async fn account_balance(account_balance_request: AccountBalanceRequest, options: Options) -> Result<AccountBalanceResponse, ApiError> {
    ...
    let tx = libra.get_transactions(metadata.version, 1, false)
        .await?;
